...
class ResidualBlock2D(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.res = nn.Sequential(
            ...
            nn.BatchNorm2d(in_ch),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_ch, in_ch, (3, 3), padding=1),

            nn.BatchNorm2d(in_ch),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_ch, out_ch, (1, 1)),
        )
        self.conv = nn.Conv2d(self.in_ch,self.out_ch,(1,1))
...
class AttentionBlock2D(nn.Module):
    def __init__(self, in_ch, out_ch, depth):
        ...
        self.res1 = ResidualBlock2D(in_ch,in_ch)

        # Trunc Branch
        self.res2 = ResidualBlock2D(in_ch,in_ch)
        self.res3 = ResidualBlock2D(in_ch,in_ch)

        # Soft Mask Branch
        ## encoder
        self.pool1 = nn.MaxPool2d(2)
        self.res4 = ResidualBlock2D(in_ch,in_ch)

        self.net1 = nn.Sequential()
        for i in range(depth):
            self.net1.add_module('res1_{index}'.format(index=i),ResidualBlock2D(in_ch,in_ch))
            self.net1.add_module('pool{index}'.format(index=i),nn.MaxPool2d(2))
            self.net1.add_module('res2_{index}'.format(index=i),ResidualBlock2D(in_ch,in_ch))

        ##decoder
        self.net2 = nn.Sequential()
        for i in range(depth):
            self.net2.add_module('res1_{index}'.format(index=i),ResidualBlock2D(in_ch,in_ch))
            self.net2.add_module('up{index}'.format(index=i),nn.Upsample(scale_factor=2))

        #last upsampling
        self.res5 = ResidualBlock2D(in_ch,in_ch)
        self.up = nn.Upsample(scale_factor=2)

        self.conv1 = nn.Conv2d(in_ch, in_ch, (1,1))
        self.conv2 = nn.Conv2d(in_ch, in_ch, (1,1))
        self.activation = nn.Sigmoid()

        self.res6 = ResidualBlock2D(in_ch, out_ch)
